// frontend/src/utils/subnetMeta_clean.js
// Static catalog of Bittensor subnets — name, description, and category.
// Runtime source of truth is the SubnetConfig collection (fetched from the API);
// this is a display fallback for subnets not yet configured in the DB.
// Keep this in sync with backend/src/utils/subnetMeta.js.
...
export function extractCleanName(channelName) {
...
export function getSubnetMeta(subnetNumber, aiGeneratedName, channelName) {
  const meta = SUBNET_META[subnetNumber];
  const name =
    meta?.name ||
    aiGeneratedName ||
    extractCleanName(channelName) ||
    `Subnet ${subnetNumber}`;
